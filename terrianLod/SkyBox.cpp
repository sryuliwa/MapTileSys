...
CSkyBox::CSkyBox():length(400.0f),width(400.0f),height(300.0f),yRot(0.01f)
{	
	m_iDrawMode = TEXTURE;
	init();
}
...
bool CSkyBox::init()
{
	...
	char *bmpName[] = { "back","front","top","left","right","bottom"};
	for(int i=0; i< 6; i++)
	{
...
void CSkyBox::render(CCamera* cam)
{
	if (m_iDrawMode != TEXTURE) //天空盒仅纹理模式下出现
	{
		return;
	}
